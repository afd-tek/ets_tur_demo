...
public class PersonValidator {
    ...
    public static boolean RuleForEmail(String email){
        return email != null && email.length() <= 60 && Regex.VALID_EMAIL_ADDRESS_REGEX.matcher(email).find();
    }

    public static boolean RuleForPhone(String phone){
        return phone != null && ((phone.length() <= 13 && Regex.VALID_PHONE_NUMBER_REGEX.matcher(phone).find()) || phone.length() <= 20);
    }

    public static boolean RuleForNote(String note){
        return note == null || note.length() <= 100;
    }
